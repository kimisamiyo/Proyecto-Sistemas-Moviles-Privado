  ...
  const openNotification = useCallback((item) => {
    ...
    try {
      switch (action.kind) {
        case 'event':
          if (!action.eventId) {
            Alert.alert('Notificación', item.body || item.title || 'Evento no disponible.');
            return;
          }
        ...
        case 'squad':
          if (!action.squadId) {
            Alert.alert('Notificación', item.body || item.title || 'Escuadra no disponible.');
            return;
          }
      ...
      Alert.alert('Notificación', item.body || item.title || 'No se pudo abrir.');
    }
  }, [navigation]);
